...
Authors: Nathan Leniz

Modified: September 2018

Building the frontend application for bundling with M220-*

To build the UI for packaging with an M220 course variant, follow the steps
outlined below.

1. Run `npm run build` to create a distributable build of the UI.
2. Copy the entire generated **build** directory into the release folder for the
   **course**. For example, when building a release of M220P, I copy the build
   folder into the `mflix` directory.
3. At this point it is now framework specific. The API server needs to respond
   to requests for static assets (css, images, js) and look in the build directory
   for them. It also needs to serve the `build/index.html` file when when it gets
   a request to the base route, **/** (`localhost:5000`).

4. You can see an example of this in the `src/handouts/m220/release/mflix-python`
   folder, look in **factory.py**.

The rest of the routing will be handled by the already defined routes in the API
as well as the UI.
